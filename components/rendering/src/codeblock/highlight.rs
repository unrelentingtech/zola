...
use syntect::html::{styled_line_to_highlighted_html, tokens_to_classed_spans, IncludeBackground, ClassStyle};
...
impl<'config> SyntaxHighlighter<'config> {
    ...
    pub fn pre_classes(&self) -> Option<String> {
        ...
        match self {
            Classed(_) => if let ClassStyle::SpacedPrefixed {prefix} = CLASS_STYLE {
